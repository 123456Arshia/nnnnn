...
class Doctor(models.Model):
    ...
    # NEW: Link each Doctor to a User account for login
    ...
    def get_available_slots(self):
        """
        This is an example method that returns static slots
        (16:00-21:00 on Saturdays).
        Consider integrating with the 'Schedule' model or
        customizing for each doctor.
        """
        available_slots = defaultdict(list)
        today = timezone.localdate()

        for day_offset in range(7):  # For the next 7 days
            day = today + timedelta(days=day_offset)
            # Example: only handle Saturday (weekday=5)
            if day.weekday() == 5:  
    ...
    def get_specialization_display(self):
        # Returns the raw specialization code. Customize if needed.
        return self.specialization

    def get_reserved_slots(self):
        """
        Return a dictionary of all 'Confirmed' appointment slots for this doctor.
        Example format: { 'Saturday': ['16:00', '16:30', ...], ... }
        """
        reserved_slots = {}
...
class Appointment(models.Model):
    ...
    def reserve_slot(cls, doctor, patient, date_time):
        ...
        # Check if patient already has a slot with this doctor
        ...
        # Check if patient is trying to book the same time with another doctor
